...
pub async fn get_resource_snapshot(
    ...
    let self_pid_raw = sysinfo::get_current_pid()
        .map_err(|e| format!("could not resolve current pid: {e}"))?;
    let self_pid_i32 = self_pid_raw.as_u32() as i32;

    // ── Refresh + collect process tree ────────────────────────────
    let now = Instant::now();
    let (snapshot_core, sampled_at) = {
        let mut sys = state.sys.lock().map_err(|e| format!("sys lock poisoned: {e}"))?;
        let mut last = state
            .last_refresh
            .lock()
...
fn sample_gpu_macos(cache: &Mutex<Option<(Instant, Option<f32>)>>) -> Option<f32> {
    ...
    let raw = run_with_timeout("ioreg", &["-r", "-d", "1", "-c", "AGXAccelerator", "-w", "0"], 800)?;
